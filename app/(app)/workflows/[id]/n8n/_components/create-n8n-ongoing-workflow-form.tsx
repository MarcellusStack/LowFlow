...
import { createN8nCompleteWorkflow, createN8nOngoingWorkflow } from "../_actions";
import { EntitySelect } from "@components/entity-select";
import { getN8nsAction } from "@server/_actions";
import { processN8nSchema } from "../_schemas";

export const CreateN8nOngoingWorkflowForm = ({ id }: { id: string }) => {
  const form = useForm({
    name: "create-n8n-ongoing-workflow-form",
    mode: "uncontrolled",
    initialValues: {
      n8nWorkflow: {
        id: "",
  ...
  const { isPending, execute } = useEnhancedAction({
    action: createN8nOngoingWorkflow,
    hideModals: true,
  });
  ...
  return (
    ...
    >
      <Stack gap="sm">
        <EntitySelect
          formActionName="create-n8n-ongoing-workflow-form"
